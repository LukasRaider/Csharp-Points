...
class TestPoint
{
    public static void Mainx()
    {
        ...
        //součet obvodů všech vytvořených kruhů a obdélníků
        double totalPerimeter = 0;
        for (int i = 0; i < shapes.Length; i++)
        {
            totalPerimeter += shapes[i].perimeter();
        }
        Console.WriteLine($"Celkovy soucet obvodu kruhu a obdelniku v poli je {{0:f}} ", totalPerimeter);
        ...
        //shapes2.ForEach(sh => total += sh.perimeter()); //LINQ
        Console.WriteLine($"Celkovy soucet obvodu kruhu a obdelniku v kolekci je {{0:f}}", total);

        //pomoci ArrayListu i s inicializatorem
        ArrayList shapes3 = new ArrayList() { c1, c2, r2, r3 };
        double totalArrayList = 0;
        foreach (Shape shape in shapes3)
        {
            totalArrayList += shape.perimeter();       //nebo by to slo pomoci typu var, ale s pretypovanim
        }

        Console.WriteLine($"Celkovy soucet obvodu kruhu a obdelniku v kolekci ArrayList je {{0:f}}", totalArrayList);
